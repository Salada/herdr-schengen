...
[] [Bug/OpenCode] OpenCode 승인 지침(Instruction Delivery) 전달 유실 및 연쇄 명령 다이얼로그 전이 불일치 (사례: #3615, #3623):
  - 현상 및 원인 (사례: Escalation #3615 `herdr agent read` 연쇄 실행 & Escalation #3623 `access_directory` vs `ls -d` 경합):
    ...
    • 근본 원인 (3가지 요인):
      ...
      2) **연쇄 명령 실행 시 Dialog Trampoline/TOCTOU 불일치 (사례 #3623)**: 에이전트가 파일 읽기(`Read AGENTS.md` -> `access_directory`)와 쉘 명령(`ls -d ...`)을 연속 트리거할 때, Watcher가 큐잉한 명령(`ls -d`)과 실시간 터미널 화면의 모달(`Access external directory`)이 서로 어긋나 `INJECT_SKIP_CHANGED`가 발동되어 승인 키 및 지침이 누락됨.
