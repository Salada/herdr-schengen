...
def _parse_json_verdict(content_str: str, prefix: str = "[Cloud Judge]") -> Optional[Tuple[bool, str]]:
...
def _post_cloud_judge(messages, endpoint, model, api_key, reasoning_effort, tools=None):
...
def audit_with_cloud_judge(
    ...
    try:
        data = _post_cloud_judge(messages, target_endpoint, target_model, target_key, reasoning_effort)
        content_str = data["choices"][0].get("message", {}).get("content", "")
        parsed = _parse_json_verdict(content_str)
