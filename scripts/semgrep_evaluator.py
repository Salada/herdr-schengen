...
PIPED_REMOTE_EXEC_PATTERN = re.compile(
    r"\b(curl|wget|fetch|http)\b[^|;\n]+\|\s*(sudo\s+|env\s+|/bin/|/usr/bin/)?(ba|z|da|a)?sh\b", re.IGNORECASE
)
...
UNSAFE_SUBPROCESS_SHELL_PATTERN = re.compile(
    r"\bsubprocess\.(Popen|run|call|check_output|check_call)\s*\([^)]*shell\s*=\s*True[^)]*\)", re.IGNORECASE
)


SCRIPT_INVOCATION_PATTERN = re.compile(
    r"\b(python[0-9.]*\s+(-c|-m)|bash\s+-c|sh\s+-c|zsh\s+-c|perl\s+-e|ruby\s+-e|node\s+-e|eval\b)", re.IGNORECASE
)


def audit_script_with_semgrep(cmd_str: str, timeout_sec: float = 0.2) -> tuple[bool, str, dict[str, Any]]:
    ...
    if PIPED_REMOTE_EXEC_PATTERN.search(cmd_str):
        return (
            False,
            "SAST Semgrep [BLOCK]: Unverified remote script piped directly to shell (curl | sh)",
            {"rule": "piped-remote-script-execution", "consequence": "PERS"},
        )

    if REVERSE_SHELL_PATTERN.search(cmd_str):
        return (
            False,
            "SAST Semgrep [BLOCK]: Reverse shell or raw socket redirection signature detected",
            {"rule": "reverse-shell-socket-redirection", "consequence": "PERS"},
        )

    if UNSAFE_SUBPROCESS_SHELL_PATTERN.search(cmd_str):
        return (
            False,
            "SAST Semgrep [BLOCK]: Python subprocess execution with shell=True vulnerability",
            {"rule": "python-unsafe-subprocess-shell", "consequence": "INT"},
        )
    ...
    if not semgrep_bin:
        return (
            True,
            "Semgrep SAST: Clean (Binary absent on script context, AST fallback active)",
